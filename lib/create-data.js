...
Array.from([...range(0, totalPatients)]).forEach(() => {
  ...
  // Parents
  const parent1 = generateParent(patient, true)
  context.parents[parent1.uuid] = parent1
  patient.parent_uuids.push(parent1.uuid)

  if (faker.datatype.boolean(0.5)) {
    const parent2 = generateParent(patient)
    context.parents[parent2.uuid] = parent2
    patient.parent_uuids.push(parent2.uuid)
  }
...
Array.from([...range(0, 20)]).forEach(() => {
  ...
  // Parents
  const parent1 = generateParent(pdsRecord, true)
  context.parents[parent1.uuid] = parent1
  pdsRecord.parent_uuids.push(parent1.uuid)

  if (faker.datatype.boolean(0.5)) {
    const parent2 = generateParent(pdsRecord)
    context.parents[parent2.uuid] = parent2
    pdsRecord.parent_uuids.push(parent2.uuid)
  }
...
for (const patientSession of Object.values(context.patientSessions)) {
  ...
  if (getConsentForPatient && !patient.hasNoContactDetails) {
    ...
    Array.from([...range(0, maxReplies)]).forEach((_, index) => {
      ...
      const parent = generateParent(patient, index === 0)
...
generateDataFile('.data/parents.json', context.parents)
