...
// Anchored to today so the session is always upcoming (planned, consent
// window open) whenever the data is regenerated.
const mmrCatchupSessionDate = setMidday(addDays(today(), 14))
const mmrCatchupOpenAt = addDays(today(), -28)
...
for (const patient_uuid of mmrCohortUuids) {
  ...
  // Register the parent as a contact so replies can resolve it. A reply given
  // online only counts as delivered if its contact got the invitation.
  context.contacts[p.parent1.uuid] = p.parent1
  if (!p.contact_uuids.includes(p.parent1.uuid)) {
    p.contact_uuids.push(p.parent1.uuid)
  }
}
...
const dmitriConsent = new Consent(
  {
    ...
    child: dmitri,
    parent: dmitriParent,
    contact_uuid: dmitriParent.uuid,
