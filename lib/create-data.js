...
// Sessions
context.sessions = {}
context.clinicVaccinationPeriods = {}
for (const preset of Object.values(SessionPresets)) {
  ...
  for (const clinic_id of clinic_ids) {
    ...
    if (clinicSession) {
      const vaccinationPeriods = generateClinicVaccinationPeriods(
        clinicSession.id,
        clinicSession.date
      )

      vaccinationPeriods.filter(Boolean).forEach((vaccinationPeriod) => {
        context.clinicVaccinationPeriods[vaccinationPeriod.uuid] =
          new ClinicVaccinationPeriod(vaccinationPeriod, context)
      })

      clinicSession.vaccination_period_ids = vaccinationPeriods.map(
        (vaccinationPeriod) => vaccinationPeriod.uuid
      )
...
generateDataFile(
  '.data/clinic-vaccination-periods.json',
  context.clinicVaccinationPeriods
)
