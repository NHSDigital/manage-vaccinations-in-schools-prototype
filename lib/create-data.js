...
// Nurse users
const nurse = User.findAll(context).find(({ role }) => role === UserRole.Nurse)
...
// Request consent and get responses
context.consentRequests = {}
context.replies = {}
for (const patientSession of PatientSession.findAll(context)) {
  const { patient, session } = patientSession

  const patientProgramme = patient.programmes[patientSession.programme_id]

  let sendConsentRequest
  switch (true) {
    ...
    case session.consentWindow === ConsentWindow.Closed:
      sendConsentRequest = faker.datatype.boolean(0.95)
      break
    default:
      sendConsentRequest = faker.datatype.boolean(0.75)
  }

  // 2️⃣🅰️ SEND CONSENT REQUEST
  if (sendConsentRequest) {
    for (const contact of patient.contacts) {
      if (!contact.hasContactDetails) {
        continue
      }

      let consentRequest = ConsentRequest.create(
        {
          createdAt: session.consentOpenAt,
      ...
      consentRequest = ConsentRequest.findOne(consentRequest.uuid, context)

      patient.requestConsent(consentRequest)
    }
  }

  for (const consentRequest of patientProgramme.consentRequests) {
    // 2️⃣🅱️ GET CONSENT RESPONSE
    let consent = generateConsent(patientSession, consentRequest)
    consent = new Consent(consent, context)

    if (consent?.child?.dob) {
      const canMatchReplyWithPatient = faker.datatype.boolean(0.95)
      if (!canMatchReplyWithPatient && session.isPlanned) {
...
for (const patient of Patient.findAll(context)) {
  ...
  const appointment = generateClinicAppointment(patient, session, clinicBooking)
...
generateDataFile('.data/consent-requests.json', context.consentRequests)
