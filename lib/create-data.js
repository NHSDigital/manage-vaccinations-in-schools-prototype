  ...
  if (getConsentForPatient && !patient.hasNoContactDetails) {
    ...
    range(0, maxReplies).forEach((_, index) => {
      ...
      if (contact.tel && faker.datatype.boolean(0.2)) {
        contact.hasCommunicationNeeds = true
        contact.communicationNeeds =
...
for (const patientSession of PatientSession.findAll(context)) {
  ...
  if (session.isCompleted) {
    ...
    const canInstruct =
      patientSession.patientProgramme.status !== PatientStatus.Triage

    if (session.hasPsdProtocol && canInstruct) {
      const instruction = generateInstruction(programme, session, nurse)
  ...
  if (session.isCompleted) {
    ...
    for (const programme of session.programmes) {
      if (
        ...
        patientSession.patientProgramme.status === PatientStatus.Due
      ) {
...
for (const patient of Patient.findAll(context)) {
  ...
  context.vaccinations[tetanusDose1.uuid] = tetanusDose1
  context.vaccinations[tetanusDose2.uuid] = tetanusDose2
  context.vaccinations[tetanusDose3.uuid] = tetanusDose3
  context.vaccinations[tetanusDose4.uuid] = tetanusDose4
}
...
for (const patient of Patient.findAll(context)) {
  if (patient?.pendingChanges?.school_id) {
    const move = new Move({
      ...
      team_id: matchingIndex === 0 ? Team.findAll(context)[0].code : undefined,
...
if (vaccinatedPatient) {
  for (const patientSession of vaccinatedPatient.patientSessions) {
    ...
    const givenConsentReply = patientSession.patientProgramme.responses.find(
