...
Array.from([...range(0, totalClinicBookings)]).forEach(() => {
...
for (let booking of Object.values(context.clinicBookings)) {

  // Create the first appointment for the booking, which will determine the parent details for the booking
  const appointment = generateClinicAppointment(context)
  booking.addAppointment(appointment)
  context.clinicAppointments[appointment.uuid] = appointment

  // Make any additional appointments for this booking
  const additionalAppointmentsCount = faker.datatype.boolean(0.9) ? 0 : faker.number.int({ min: 1, max: 3 })
  for (let i = 0; i < additionalAppointmentsCount; i++) {
    ...
    const nextAppointment = generateClinicAppointment(context)
