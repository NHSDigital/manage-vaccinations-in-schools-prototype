...
let programme
context.replies = {}
for (const patientSession of Object.values(context.patientSessions).filter(
  (patientSession) => patientSession.session.type === SessionType.School
)) {
...
// Clinic invites for children who are clinic ready e.g. home-educated or missed school session
for (const patient of Patient.findAll(context)) {
  const clinicReadyProgramme_ids = Object.values(patient.programmes)
