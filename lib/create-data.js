...
import { generateClinicBooking } from '../app/generators/clinic-booking.js'
...
Array.from([...range(0, totalClinicBookings)]).forEach(() => {
  const booking = generateClinicBooking(context)
  context.clinicBookings[booking.uuid] = booking
})

// TODO: create empty bookings first, then loop through them and add the appointments here, putting each appointment in context.clinicAppointments
