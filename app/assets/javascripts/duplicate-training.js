...
const checkedRadio = () => form.querySelector('input[name="preferred-record"]:checked')
form.addEventListener("submit", (event) => {
  ...
  form.querySelector("[data-choose-group]").classList.add("nhsuk-form-group--error")
  form.querySelector("fieldset").setAttribute("aria-describedby", "preferred-record-error")
  summary.focus()
})
...
if (bar) {
  ...
  for (const radio of radios) radio.addEventListener("change", () => write(CHOICE_KEY, radio.value))
  ...
  const buttons = { skip: q('[data-set-action="skip"]'), submit: q('[data-set-action="submit"]'), comment: q('[data-set-action="comment"]') }
  const showButtons = () => {
  ...
  const openPanel = () => {
    q("[data-comment-question]").textContent = action === "skip" ? "Why did you skip this set?" : "Why did you choose this record?"
    if (panel.hidden) { panel.hidden = false; showButtons(); measureBar() }
    field.focus()
  }
  ...
  buttons.skip.addEventListener("click", (event) => { action = "skip"; if (!toggle.checked) { finish(); return } event.preventDefault(); openPanel() })
  buttons.comment.addEventListener("click", (event) => { event.preventDefault(); form.requestSubmit() })
  // Runs after the validation listener above, so only a valid submit reaches here
  form.addEventListener("submit", (event) => { if (event.defaultPrevented) return; action = "submit"; if (!toggle.checked) { finish(); return } event.preventDefault(); openPanel() })
  q("[data-comment-cancel]").addEventListener("click", (event) => { event.preventDefault(); closePanel(); (action === "skip" ? buttons.skip : buttons.comment).focus() })
  ...
  q("[data-comment-form]").addEventListener("submit", (event) => {
    event.preventDefault()
    const entry = { decision: action === "skip" ? "skip" : checkedRadio().value, comment: field.value }
